...
def plot_dones(episodes, done_array, game):
    sns.set(style="darkgrid")
    # Create the data
...
def plot_success_rate(games, dones, game):
    sns.set(style="darkgrid")
    # Create the data
    x = [k + 1 for k in range(game+1)]
...
if __name__ == '__main__':
    ...
    lr = 0.0005
    n_games = 3000
    agent = Agent(gamma=0.99, epsilon=0.01, lr=lr, input_dims=env.observation_space.shape,
    ...
    for i in range(n_games):
        ...
        if i % 50 == 0:
            ...
            plot_learning(n_games, scores, eps_history)
            plot_success_rate(n_games, dones, i)
    agent.save_weights(str(i) + '_episodes_New_R_target')
    agent.save_model(str(i))
